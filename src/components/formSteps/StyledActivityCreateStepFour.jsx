...
const ActivityCreateStepFour = ({
  ...
  const checkIsStepFourComplete = () => {
    ...
    for (let i = 0; i < formContent?.accommodation?.length; i++) {
      ...
      if (accommodationDay) {
        if (
          accommodationDay?.date &&
          updatedFormErrors?.accommodation?.[i]?.date
        )
          updatedFormErrors.accommodation[i].date = '';
        if (
          accommodationDay?.name &&
          updatedFormErrors?.accommodation?.[i]?.name
        )
          updatedFormErrors.accommodation[i].name = '';
        if (
          accommodationDay?.address &&
          updatedFormErrors?.accommodation?.[i]?.address
        )
          updatedFormErrors.accommodation[i].address = '';
        if (
          accommodationDay?.roomDetail &&
          updatedFormErrors?.accommodation?.[i]?.roomDetail
        )
          updatedFormErrors.accommodation[i].roomDetail = '';
        if (
          accommodationDay.notes &&
          updatedFormErrors.accommodation?.[i]?.notes
        )
          updatedFormErrors.accommodation[i].notes = '';
      }
    }
  ...
  return (
    <div className={clsx(className, 'scrollbar')}>
      ...
      <div className="c-activity-create__accommodation-buttons">
        {accommodationList?.length > 0 && (
          <div
            className="c-activity-create__accommodation-minus-button"
        ...
        <div
          className="c-activity-create__accommodation-add-button"
