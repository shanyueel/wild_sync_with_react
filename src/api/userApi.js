...
export const getUser = async (userId) => {
  try {
    const userInfo = (
      await getDoc(doc(firestoreDB, 'users', `${userId}`))
    )?.data();
...
export const buildUser = async (userId, accountInfo) => {
  try {
    await setDoc(doc(firestoreDB, 'users', `${userId}`), {
      ...
      region: '以山為家',
      introduction:
        '大家好！我是山林探險家，喜歡攀登高峰，感受山川的氣息。期待在Wild Sync認識志同道合的山友，一起探索未知的頂峰！',
      heldActivities: [],
      attendedActivities: [],
      likedActivities: [],
    });
  ...
  try {
    if (userId || updateContent) {
      ...
      for (let updateKey in updateContent) {
        ...
        if (updateProfileFilter.includes(updateKey))
          updateProfileContent[updateKey] = updateContent[updateContent];
      }

      await setDoc(doc(firestoreDB, 'users', `${userId}`), updateDocContent, {
        merge: true,
      });
