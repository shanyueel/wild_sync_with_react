...
export const getActivity = async (activityId) => {
  try {
    const activity = (
      await getDoc(doc(firestoreDB, 'activities', activityId))
    )?.data();
    if (activity?.holder)
      activity['holder'] = await getActivityHolder(activity?.holder);
    if (activity?.detail)
      activity['detail'] = (await getDoc(activity?.detail))?.data();
    if (activity?.transportation)
      activity['transportation'] = (
        await getDoc(activity?.transportation)
      )?.data();
    if (activity?.accommodation)
      activity['accommodation'] = (
        await getDoc(activity?.accommodation)
...
export const getOrderedActivities = async (orderType) => {
  try {
    ...
    const orderedActivitiesIdList = (await getDocs(orderQuery))?.docs?.map(
      (doc) => doc?.data()?.id
    );
...
export const getActivitiesByTimeFilter = async (timeFilter) => {
  try {
    ...
    const startQuery = timeFilter?.start
      ? query(
          collection(firestoreDB, 'activities'),
          where('time.start', '>=', timeFilter?.start)
        )
      : null;
    const endQuery = timeFilter?.end
      ? query(
          collection(firestoreDB, 'activities'),
          where('time.end', '<=', timeFilter?.end)
        )
      : null;
...
export const getActivitiesByFilters = async (filters) => {
  try {
    ...
    if (filters?.time) {
      ...
      filteredActivitiesIdList = filteredActivitiesIdList?.filter((id) =>
        timeFilteredIdList?.includes(id)
      );
    }
    ...
    if (filters?.location?.length > 0) {
      const locationQuery = query(
        collection(firestoreDB, 'activities'),
        where('location.county', 'in', filters?.location)
      );
      const locationFilteredIdList = (await getDocs(locationQuery))?.docs?.map(
        (doc) => doc?.data()?.id
      );
      filteredActivitiesIdList = filteredActivitiesIdList?.filter((id) =>
        locationFilteredIdList?.includes(id)
      );
    }
    ...
    if (filters?.difficulty?.length > 0) {
      ...
      const difficultyFilteredIdList = (
        await getDocs(difficultyQuery)
      )?.docs?.map((doc) => doc?.data()?.id);
      filteredActivitiesIdList = filteredActivitiesIdList?.filter((id) =>
        difficultyFilteredIdList?.includes(id)
      );
    }
    ...
    if (filters?.keyword) {
      ...
      await asyncForEach(filteredActivitiesIdList, async (activityId) => {
        const name = (await getDoc(doc(firestoreDB, 'activities', activityId)))
          ?.data()
          ?.name?.toLowerCase();
        const detail = (
          await getDoc(
            doc(firestoreDB, 'activities-details', `${activityId}-detail`)
          )
        )?.data();
    ...
    const filteredActivities = await getActivitiesByIdList(
      filteredActivitiesIdList
    );
...
export const getPopularLocations = async () => {
  try {
    const locationsOrdersQuery = query(
      collection(firestoreDB, 'activities'),
      orderBy('location.county')
    );
    const activitiesLocations = (
      await getDocs(locationsOrdersQuery)
    )?.docs?.map((doc) => doc?.data()?.location?.county);

    const countiesCounts = activitiesLocations?.reduce(
    ...
    const popularLocations = countiesCountsResult
      .map((county) =>
        taiwanDistricts.find((taiwanCounty) => taiwanCounty.id === county)
      )
      .slice(0, 6);
...
export const postActivity = async (
  ...
  try {
    ...
    if (detailContent) {
      ...
      mainActivityContent['detail'] = doc(
        firestoreDB,
        'activities-details',
        `${activityId}-detail`
      );
    ...
    if (transportationContent) {
      await setDoc(
        doc(
          ...
          `${activityId}-transportation`
        ),
        transportationContent
      );
      mainActivityContent['transportation'] = doc(
        firestoreDB,
        'activities-transportation',
        `${activityId}-transportation`
      );
    ...
    if (accommodationContent?.accommodationList?.length > 0) {
      ...
      mainActivityContent['accommodation'] = doc(
        firestoreDB,
        'activities-accommodation',
        `${activityId}-accommodation`
      );
    ...
    await setDoc(
      doc(firestoreDB, 'activities', `${activityId}`),
      mainActivityContent
    );
...
export const updateActivity = async (
  ...
  try {
    const nowTimeString = Date.parse(new Date());
    let [detailUpdate, transportationUpdate, accommodationUpdate] = [
      null,
      null,
      null,
    ];
    ...
    if (updateContent?.accommodation?.length > 0) {
      ...
      if (currentActivity?.accommodation) {
        ...
        updateContent['accommodation'] = doc(
          firestoreDB,
          'activities-accommodation',
          `${activityId}-accommodation`
        );
      ...
      !updateContent?.accommodation?.length > 0 ||
      currentActivity?.accommodation
    ) {
      await deleteDoc(
        doc(
          firestoreDB,
...
export const alterActivityAttendance = async (userId, activityId) => {
  try {
    ...
    if (existingAttendance.includes(userId)) {
      const newAttendance = existingAttendance.filter(
        (attendance) => attendance !== userId
      );
      const newAttendedActivities = currentAttendedActivities.filter(
      ...
      if (
        Number(activityRef?.attendance?.length) ===
          Number(activityRef?.attendanceLimit) ||
        Date.parse(now) > activityRef?.deadline
      )
        return { success: false };
...
export const removeAttendance = async (activity, removeAttendanceId) => {
  try {
    ...
    const newAttendance = oldAttendance?.filter(
      (attendance) => attendance !== removeAttendanceId
    );
    const oldAttendedActivities = userRef?.attendedActivities;
    const newAttendedActivities = oldAttendedActivities?.filter(
      (attendedActivity) => attendedActivity !== activityId
    );
...
export const alterActivityLiked = async (userId, activityId) => {
  try {
    ...
    if (!currentLikedActivities?.includes(activityId)) {
      const newLikedActivities = [...currentLikedActivities, activityId];
      ...
      const newLikedActivities = currentLikedActivities?.filter(
        (activity) => activity !== activityId
      );
      await updateDoc(doc(firestoreDB, 'users', `${userId}`), {
        likedActivities: newLikedActivities,
...
export const deleteActivity = async (userId, activity) => {
  try {
    if (activity?.detail)
      await deleteDoc(
        doc(firestoreDB, 'activities-details', `${activity?.id}-detail`)
      );
    if (activity?.transportation)
      await deleteDoc(
        doc(
    ...
    const newHeldActivities = currentHeldActivities?.filter(
      (heldActivity) => heldActivity !== activity?.id
    );
    await updateDoc(doc(firestoreDB, 'users', `${userId}`), {
