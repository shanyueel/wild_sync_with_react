...
const RangeInput = ({
  ...
  const [range, setRange] = useState(
    formContent?.[inputId] || { min: null, max: null }
  );
  ...
  const handleMinInput = () => {
    const minValue =
      minRef?.current?.value === '' ? null : Number(minRef?.current?.value);
  ...
  const handleMaxInput = () => {
    const maxValue =
      maxRef?.current?.value === '' ? null : Number(maxRef?.current?.value);
  ...
  return (
    <div className={className}>
      ...
      <div className="c-input-body">
        ...
        <div className="o-input-body__unit">{unit}</div>
      </div>
    </div>
  );
};
