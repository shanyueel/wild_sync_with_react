  ...
  const handleUpdate = async () => {
    ...
    if (isFormComplete) {
      if (userContent?.photoURLFile) {
        const updatePhotoURL = await uploadImage(
          'avatars',
          `${userId}-avatar`,
          userContent?.photoURLFile
        );
      ...
      if (userContent?.coverURLFile) {
        const updateCoverURL = await uploadImage(
          'user-covers',
          `${userId}-cover`,
          userContent?.coverURLFile
        );
      ...
      for (let contentKey in updateUserData) {
        if (updateUserData?.[contentKey] === user?.[contentKey])
          delete updateUserData?.[contentKey];
      }
