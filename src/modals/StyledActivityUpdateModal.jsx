...
const ActivityUpdateModal = ({
  ...
  const handleNextPageClick = async () => {
    if (formProgress === 1) {
      ...
      if (!activityContent?.time?.start || !activityContent?.time?.end)
        newFormErrors.time = missingError;
    ...
    if (formProgress === 2) {
      ...
      if (!activityContent?.activityTimeLength)
        newFormErrors.activityTimeLength = missingError;
      if (!activityContent?.cost) newFormErrors.cost = missingError;
      if (!activityContent?.attendanceLimit)
        newFormErrors.attendanceLimit = missingError;
      if (!activityContent?.deadline) newFormErrors.deadline = missingError;
      if (!activityContent?.introduction)
        newFormErrors.introduction = missingError;
      setFormErrors(newFormErrors);
    ...
    if (formProgress === 3) {
      const isStepThreeComplete =
        activityContent.detail.trackType === 'one-way'
      ...
      const newFormErrors = {};
      if (!activityContent?.detail?.departurePoint)
        newFormErrors.departurePoint = missingError;
      if (!activityContent?.detail?.trackType)
        newFormErrors.trackType = missingError;
      if (!activityContent?.detail?.exitPoint)
        newFormErrors.exitPoint = missingError;
      if (!activityContent?.detail?.trackLength)
        newFormErrors.trackLength = missingError;
      if (!activityContent?.detail?.altitude)
        newFormErrors.altitude = missingError;
      if (!activityContent?.detail?.trackCondition)
        newFormErrors.trackCondition = missingError;
      if (!activityContent?.detail?.belongingPark)
        newFormErrors.belongingPark = missingError;
      if (!activityContent?.detail?.applicationNeeded)
        newFormErrors.applicationNeeded = missingError;
      if (!activityContent?.detail?.trackIntroduction)
        newFormErrors.trackIntroduction = missingError;
    ...
    if (formProgress === 4) {
      ...
      for (let i = 0; i < accommodation.length; i++) {
        ...
        isStepFourComplete =
          isStepFourComplete && isCurrentAccommodationComplete;
      }

      const newFormErrors = {};
      if (!activityContent?.transportation?.outbound)
        newFormErrors.outbound = missingError;
      if (!activityContent?.transportation?.inbound)
        newFormErrors.inbound = missingError;
      ...
      for (let i = 0; i < accommodation.length; i++) {
        ...
        if (!currentAccommodation?.date)
          newFormErrors.accommodation[i].date = missingError;
        if (!currentAccommodation?.name)
          newFormErrors.accommodation[i].name = missingError;
        if (!currentAccommodation?.address)
          newFormErrors.accommodation[i].address = missingError;
        if (!currentAccommodation?.roomDetail)
          newFormErrors.accommodation[i].roomDetail = missingError;
        if (!currentAccommodation?.notes)
          newFormErrors.accommodation[i].notes = missingError;
      }
    ...
    for (let updateKey in filteredUpdateContent) {
      if (filteredUpdateContent?.[updateKey] === currentActivity?.[updateKey]) {
        ...
        for (let detailKey in filteredUpdateContent?.detail) {
          if (
            filteredUpdateContent?.detail?.[detailKey] ===
            currentActivity?.detail?.[detailKey]
          ) {
        ...
        for (let transportationKey in filteredUpdateContent?.transportation) {
          if (
            filteredUpdateContent?.transportation?.[transportationKey] ===
            currentActivity?.transportation?.[transportationKey]
          ) {
  ...
  const handleActivityUpdate = async () => {
    ...
    const newFormErrors = {};
    if (!activityContent?.detail?.schedule)
      newFormErrors.schedule = missingError;
    setFormErrors(newFormErrors);
    ...
    if (isStepFiveComplete) {
      ...
      if (updatedActivity?.coverURLFile) {
        ...
        updateCoverURL = await uploadImage(
          'activities-covers',
          `${activityId}-cover`,
          updatedActivity?.coverURLFile
        );
      ...
      if (updatedActivity?.detail?.mapURLFile) {
        ...
        updateMapURL = await uploadImage(
          'activities-maps',
          `${activityId}-map`,
          updatedActivity?.detail?.mapURLFile
        );
      ...
      const { success, updateAt } = await updateActivity(
        activityId,
        currentActivity,
        updateData
      );
